# Package index

## Typicality tests

Exact combinatorial inference tests for the typicality of a mean point
in a geometric (GDA) cloud (Le Roux, Bienaise & Durand 2019, chapters
3–4).

- [`typicality_comb()`](https://label42.github.io/GDAinference/reference/typicality_comb.md)
  : Combinatorial typicality test for a mean point
- [`typicality_byvar()`](https://label42.github.io/GDAinference/reference/typicality_byvar.md)
  : Combinatorial typicality test for every category of a variable
- [`typicality_geom()`](https://label42.github.io/GDAinference/reference/typicality_geom.md)
  : Geometric typicality test for a mean point

## Homogeneity test

Exact combinatorial test of the homogeneity of two groups in a geometric
(GDA) cloud (Le Roux, Bienaise & Durand 2019, chapter 5).

- [`homogeneity()`](https://label42.github.io/GDAinference/reference/homogeneity.md)
  : Combinatorial homogeneity test

## Compatibility with GDA packages

Extract principal coordinates from the result objects of the common GDA
packages, so the tests work whatever the upstream tool.

- [`get_coord()`](https://label42.github.io/GDAinference/reference/get_coord.md)
  : Extract principal coordinates from a GDA result

## Methods

Print and plot methods for the test results.

- [`print(`*`<gdainference_test>`*`)`](https://label42.github.io/GDAinference/reference/print.gdainference_test.md)
  [`summary(`*`<gdainference_test>`*`)`](https://label42.github.io/GDAinference/reference/print.gdainference_test.md)
  : Print a combinatorial inference test
- [`print(`*`<typicality_byvar>`*`)`](https://label42.github.io/GDAinference/reference/print.typicality_byvar.md)
  : Print a by-category typicality table
- [`print(`*`<typicality_geom>`*`)`](https://label42.github.io/GDAinference/reference/print.typicality_geom.md)
  : Print a geometric typicality test
- [`print(`*`<homogeneity>`*`)`](https://label42.github.io/GDAinference/reference/print.homogeneity.md)
  : Print a combinatorial homogeneity test
- [`plot(`*`<typicality_comb>`*`)`](https://label42.github.io/GDAinference/reference/plot.typicality_comb.md)
  : Plot a combinatorial typicality test
- [`plot(`*`<typicality_geom>`*`)`](https://label42.github.io/GDAinference/reference/plot.typicality_geom.md)
  : Plot a geometric typicality test
- [`plot(`*`<homogeneity>`*`)`](https://label42.github.io/GDAinference/reference/plot.homogeneity.md)
  : Plot a combinatorial homogeneity test

## Data sets

Didactic data from Le Roux, Bienaise & Durand (2019).

- [`Target`](https://label42.github.io/GDAinference/reference/Target.md)
  : Target example: reference cloud
- [`Target_group`](https://label42.github.io/GDAinference/reference/Target_group.md)
  : Target example: group cloud
- [`Student`](https://label42.github.io/GDAinference/reference/Student.md)
  : Student example

## Package

- [`GDAinference`](https://label42.github.io/GDAinference/reference/GDAinference-package.md)
  [`GDAinference-package`](https://label42.github.io/GDAinference/reference/GDAinference-package.md)
  : GDAinference: Combinatorial Inference in Geometric Data Analysis
